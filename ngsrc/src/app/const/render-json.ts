...
const renderjson = (function () {
    const themetext = (...args: any[]) => {
        const spans = [];
        while (args.length) {
            spans.push(append(span(Array.prototype.shift.call(args)), text(Array.prototype.shift.call(args))));
        }
        return spans;
    };
    const append = function (...args: any[]) {
    ...
    const A = (txt, classname, callback) => {
    ...
    function _renderjson(json, indent, dont_indent, show_level, options) {
        ...
        return disclosure("{", options.collapse_msg(Object.keys(json).length), "}", "object", function () {
            ...
            for (const k in json) { const last = k; }
